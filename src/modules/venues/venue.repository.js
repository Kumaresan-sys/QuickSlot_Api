...
async function findAllVenues() {
  const result = await pool.query(
    `SELECT id, name, location, created_at FROM venues ORDER BY created_at DESC`
  );
  return result.rows;
}
...
async function findVenueById(venueId) {
  const result = await pool.query(
    `SELECT id, name, location, created_at FROM venues WHERE id = $1`,
    [venueId]
  );
...
module.exports = { findAllVenues, findVenueById, findSlotsByVenueAndDate };
